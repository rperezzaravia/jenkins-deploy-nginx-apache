        ...
        stage('Create the containers in Parallel') {
            parallel{
                ...
                {           
                    ...
                    sh 'docker run -dit --name app-web-apache -p 9100:80  -v /var/lib/docker/volumes/jenkins_home/_data/practicas/app-web:/usr/local/apache2/htdocs/ httpd'
                    }
                }
                stage('Create the Nginx container') {
                   steps {
                   echo 'Creating the Apache container...'
                   sh 'docker run -dit --name app-web-nginx -p 9200:80  -v /var/lib/docker/volumes/jenkins_home/_data/practicas/app-web:/usr/share/nginx/html nginx'         
                   }
                }       
            }   
        }
